...
namespace Primrose.Primitives.Tasks
{
  ...
  public static class TaskHandler
  {
    ...
    public static Task StartNew(this TaskSet taskset)
    {
    ...
    /// <returns>The task generated and started by this method</returns>
    public static Task StartNew<T>(this TaskSet<T> taskset, T state)
    {
    ...
    /// <param name="completion_handler">The action to be performed on successful completion of the main action</param>
    /// <returns></returns>
    public static Task StartNew<T>(Action<T> action, T state, Action<Task, T> exception_handler, Action<Task, T> completion_handler)
    {
      if (action == null) action = c => { };
      if (exception_handler == null) exception_handler = (c, t) => { };
      if (completion_handler == null) completion_handler = (c, t) => { };

      return Task.Factory.StartNew(() => action(state))
        .ContinueWith((t) => exception_handler(t, state), TaskContinuationOptions.OnlyOnFaulted | TaskContinuationOptions.ExecuteSynchronously)
        .ContinueWith((t) => completion_handler(t, state), TaskContinuationOptions.NotOnFaulted | TaskContinuationOptions.ExecuteSynchronously);
    }
